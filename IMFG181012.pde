...
void sky() {
  ...
  for (int j=-100; j<=900; j+=150) {
    for (int i=-350; i<3*mx-100; i+=150) {
      pushMatrix();
...
void flyingObj() {
  ...
  missile(700, 0, 13000);
  missile(120, -360, 15000);
  missile(-900, 0, 16000);
  missile(-300, -200, 18000);
  missile(-720, 0, 19000);
  obstacle(-100, 4, 5000);
  obstacle(300, 2, 9000);
  obstacle(-500, 3, 15000);
}

void missile(float cx, float cy, float cd) {
  if (z>cd-5000 || z<cd+50) {
...
void myself() {
  ...
  if (abs(x-px)>mx/40 || abs(y-py)>my/20) {
    x=px;
    y=py;
  }
